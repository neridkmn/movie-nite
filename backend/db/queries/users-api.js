...
const createUser = ({ name, email, adminId }) => {
  return db
    .query(`INSERT INTO users (name, email, admin_id, created_at, updated_at)
...
const addMembership = ({ userId, groupId }) => {
  return db
    .query(`INSERT INTO membership (user_id, group_id, created_at)
            VALUES ($1, $2, NOW()) 
            RETURNING *;`,
            [userId, groupId]
    )
...
const getMembershipByUserAndGroupId = ({ userId, groupId }) => {
  return db
    .query(`SELECT * FROM membership WHERE user_id = $1 AND group_id = $2`, [userId, groupId])
    .then(data => data.rows[0]);
};
