...
const addPreferredMovies = ({
  ...
  return db
    .query(
      `UPDATE membership SET preferred_movie_1 = $1, preferred_movie_2 = $2, preferred_movie_3 = $3
...
const getPreferredMoviesByGroupId = (groupId) => {
  return db
    .query(
      `SELECT preferred_movie_1, preferred_movie_2, preferred_movie_3 FROM membership 
  WHERE group_id = $1;`,
      [groupId]
    )
    .then((data) => {
...
const setSuggestedMovies = ({
  ...
  return db
    .query(
      `UPDATE groups SET suggested_movie_1 = $1, suggested_movie_2 = $2, suggested_movie_3 = $3
    WHERE id = $4
    RETURNING *;`,
...
const setVotedMovie = ({ userId, groupId, votedMovie }) => {
  return db
    .query(
      `UPDATE membership SET voted_movie = $1
...
const getVotedMoviesByGroupId = (groupId) => {
  return db
    .query(
      `SELECT voted_movie FROM membership
    WHERE group_id = $1;`,
      [groupId]
    )
    .then((data) => data.rows)
